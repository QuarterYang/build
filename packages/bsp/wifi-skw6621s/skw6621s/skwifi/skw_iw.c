...
int skw_wow_filter_set(struct net_device *ndev, u8 *param, int len, char *resp)
{
	...
	int pos = 0, ret = 0, rule_idx = 0, offset, total;
	u16 result_len = 0;
	...
	wow_param = (struct skw_wow_input_param *)((u8 *)spd + sizeof(*spd));
			...
			union iwreq_data *wrqu, char *extra)
{
	...
	skw_hex_dump("param:", param, strlen(param), false);
			...
			union iwreq_data *wrqu, char *extra)
{
	...
	ret = skw_wow_filter_set(dev, param, min_t(int, sizeof(param),
